...
| F-26 | Schätzmodus: Fibonacci oder freier Wertebereich | fertig | — | gemergt 12.09. 01:16 |
...
|---|---|---|---|
| A — F-25 Datenzoom: Features verlassen die Plotfläche statt auszublenden | fertig | — | gemergt 12.09. 20:45 |
| B — F-24 Umschalter Abdunkeln/Ausblenden gehört ins Kopfband, links neben „Hervorhebung" (nicht ins Menü „Ansicht") | fertig | — | gemergt 12.09. 20:53 |
...
| 11.–12.09. | manuell, dann geweckt (20:23) | Nutzer hat PRD 1.1 mit drei neuen Features gepusht (F-24 Hervorhebung Abdunkeln/Ausblenden, F-25 Datenzoom als Ersatz für F-12, F-26 Schätzmodus). Alle drei sequenziell durch Tests, Umsetzung, QA und Merge gebracht: F-24 (ein Testfehler durch Jitter-Kollision/FR-46-Wechselwirkung behoben), F-25 (großer Umbau: `viewport.ts`/`scales.ts` auf Fenster-Signatur umgestellt, `src/lib/store/viewport.test.ts` und `e2e/F-12-zoom-pan.spec.ts` als durch F-25 ausdrücklich abgelöst gelöscht, zwei Testfehler behoben — F-14-Zentrierung maß gegen die falsche Bounding Box, Font-Lade-Race in einem Bildschirmkonstanz-Test), F-26 (Einstellungsdialog-Kontrakt analog F-13 festgelegt, vier zusammenhängende Testfehler behoben — zweimal fehlende Persistenz-Debounce-Wartezeit, zweimal Groß-/Kleinschreibungs- bzw. Zeichensatzfehler bei Test-Kennungen). Ein Rate-Limit-Abbruch bei F-24 von frischem Feature-Agent fortgesetzt. Gesamtsuite auf main nach dem letzten Merge: 569/569 Unit-, 280/280 E2E-Tests grün. **Alle 26 Features (F-01–F-26) fertig.** | Kein offenes Feature mehr. Nächste Session: prüfen, ob der Nutzer weiteren Umfang hat, sonst Weckzyklus beenden. |
| 12.09. | manuell | Nutzer meldete zwei Bugs: (A) Features werden beim Zoomen über den Rand der Plotfläche hinausgeschoben statt ausgeblendet (Bug-Report als GitHub-Issue #3 dokumentiert); (B) der Umschalter Abdunkeln/Ausblenden soll ins Kopfband statt ins Menü „Ansicht" (ersetzt die F-24-Festlegung vom 11.09.). Beide unabhängig (keine gemeinsamen Dateien) parallel durch Tests, Umsetzung und QA gebracht. Bug A: Sichtbarkeitsprüfung in `scales.ts` ergänzt, `FeatureNodes.svelte`/`Edges.svelte` blenden Elemente außerhalb des Zoom-Fensters aus; deckte zwei falsche Testannahmen in bereits gemergten F-25/F-26-Tests auf (beide durch den bisherigen Bug maskiert), behoben. Bug B: Umschalter aus dem Menü ins Kopfband verschoben; QA fand und behob zwei Abweichungen (Kopfband überlief zwischen 1025–1377 px, Umschalter ohne `.lbl` wich von „gleicher Bauart" wie „Hervorhebung" ab). Ein Rate-Limit-Abbruch je bei Bug-B-Feature-Agent und Bug-B-QA-Agent, beide fortgesetzt. Gesamtsuite auf main nach dem letzten Merge: 575/575 Unit-, 285/285 E2E-Tests grün. | Kein offenes Feature/Bug mehr. Nächste Session: prüfen, ob der Nutzer weiteren Umfang hat, sonst Weckzyklus beenden. |
